<template>
  <div class="door-area">
    <div class="door-frame" :class="{selected: selected}">
        <GiftItem v-if="open && hasGift"/>
    </div>
    <div class="door" 
             :class="{open: open}"
             @click="selected = !selected">
        <!-- number door -->
        <div class="number" :class="{selected: selected && !open}">{{ number }}</div>
        <!-- maçaneta -->
        <div class="knob" 
                 @click="open = true"
                 :class="{selected: selected}"></div>
    </div>
  </div>
</template>

<script>
import GiftItem from './GiftItem.vue'

export default {
    name: 'DoorItem',
    components: { GiftItem },
    props: {
        number: {},
        hasGift: { type: Boolean }
    },
    // Data em Vue, sempre será uma func para que sempre retorne um novo objeto
    data: function() {
        // ESTADO INICIAL
        return {
            // não aberta e não selecionada
            open: false,
            selected: false,
        }
    }
}
</script>

<style>
:root {
    --door-border: 5px solid brown;
    --selected-border: 5px solid yellow;
}

.door-area
{
    position: relative;
    width: 200px;
    height: 310px;
    border-bottom: 10px solid #AAA;
    margin-bottom: 20px;
    font-size: 3rem;

    display: flex;
    justify-content: center;
}

.door-frame
{
    position: absolute;
    height: 300px;
    width: 180px;

    border-left: var(--door-border);
    border-top: var(--door-border);
    border-right: var(--door-border);

    display: flex;
    /* gift na parte debaixo e no centro */
    justify-content: center;
    align-items: flex-end;
}

.door-frame.selected
{
    border-left: var(--selected-border);
    border-top: var(--selected-border);
    border-right: var(--selected-border);
}
.door
{
    position: absolute;
    top: 5px;
    height: 295px;
    width: 170px;
    background-color: chocolate;

    display: flex;
    flex-direction: column;
    align-items: center;
    padding: 15px;
}

.door .knob
{
    height: 20px;
    width: 20px;
    border-radius: 10px;
    background-color: brown;
    align-self: flex-start;
    margin-top: 60px;
}

.door > .number.selected
{
    color: yellow;
}

.door > .knob.selected
{
    background-color: yellow;
}

/* transparencia de fundo, com porta aberta */
.door.open
{
    background-color: #0007;
}

/* sumir com numero e maçaneta, apos porta abrir */
.door.open .knob,
.door.open .number
{
    display: none;
}
</style>